...
int main(void)
{
	...
	result = cudaMemcpy(deviceInput1, hostInput1, size,
...
void vecAddCPU(const float *arrl, const float *arr2, float *out, int len)
{
	StopWatchInterface *timer = NULL;
	sdkCreateTimer(&timer);
	sdkStartTimer(&timer);
